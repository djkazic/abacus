...
def should_open_to_loop(lnd_client: LNDClient) -> str:
    """
    ...
    try:
        ...
        if wallet_balance_response["status"] != "OK":
            return (
                f"Error checking wallet balance: {wallet_balance_response['message']}"
            )
        ...
        if confirmed_balance < 31000000:
            return (
                f"Node has only {confirmed_balance} sats on-chain, which is not enough. "
        ...
        if channel_balance_response["status"] != "OK":
            return f"Error checking inbound liquidity: {channel_balance_response['message']}"
        ...
        if remote_balance >= 30000000:
            return (
                f"Node has {remote_balance} sats of inbound liquidity and {confirmed_balance} sats on-chain, "
            ...
            return (
                f"Node has only {remote_balance} sats of inbound liquidity, which is not enough. "
                f"The threshold is 30,000,000 sats. Continue with other channel opening strategies."
            )
    except grpc.RpcError as e:
        return f"Error checking conditions to open to loop: {e.details()}"
